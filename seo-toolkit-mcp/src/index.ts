...
import { SeoToolkitOAuthProvider } from './auth/provider.js';
...
const provider = new SeoToolkitOAuthProvider();
...
app.use(
  mcpAuthRouter({
    ...
    scopesSupported: ['seo-toolkit'],
    resourceName: 'SEO Toolkit'
  })
);
...
// token; everything below delegates to SEO Toolkit's own API and auth.
//
...
app.post('/mcp', requireAuth, express.json(), async (req, res) => {
  try {
    const server = createSeoToolkitMcpServer();
...
app.listen(config.port, () => {
  console.log(`SEO Toolkit MCP server listening on port ${config.port}`);
