  ...
  return (
    <div data-testid="deliverables-page">
      ...
      {pendingApprovals.length > 0 && (
        <Section
          title={`Awaiting your approval (${pendingApprovals.length})`}
          description="These were generated by your agents but won't appear in the deliverables backlog until you review them."
          testId="pending-approvals-section"
        >
          <div className="space-y-2">
            {pendingApprovals.slice(0, 12).map((a) => {
              const isCompetitive = a.kind === "competitive_deliverable";
              const href = isCompetitive
                ? `/clients/${clientId}/deliverables/competitive/${a.id}`
                : `/clients/${clientId}/approvals`;
              return (
                <Link
                  key={a.id}
                  to={href}
                  className="block rounded-sm border border-zinc-800 hover:border-amber-400/40 bg-zinc-900 p-3"
                  data-testid={`pending-approval-${a.id}`}
                >
                  <div className="flex items-center gap-3">
                    <span className="inline-flex items-center px-1.5 py-0.5 rounded-sm border border-amber-400/40 bg-amber-400/10 text-amber-300 font-mono text-[9px] uppercase tracking-wider">
                      pending
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-zinc-100 truncate">{a.title || a.kind}</div>
                      <div className="text-[10px] font-mono text-zinc-500 mt-0.5">
                        {KIND_META[a.kind]?.label || a.kind} · {formatRelative(a.created_at)}
                      </div>
                    </div>
